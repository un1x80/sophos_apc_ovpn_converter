...
class App:
    def __init__(self, master):
        self.master = master
        master.title("Converter APC<->OVPN")

        self.button_frame = tk.Frame(master)
        self.button_frame.pack(pady=10)

        self.btn_apc_to_ovpn = tk.Button(self.button_frame, text="Convert from APC to OVPN", command=self.run_apc_to_ovpn)
        self.btn_apc_to_ovpn.pack(side=tk.LEFT, padx=5)

        self.btn_ovpn_to_apc = tk.Button(self.button_frame, text="Convert from OVPN to APC", command=self.run_ovpn_to_apc)
    ...
    def run_script(self, script_name):
        ...
        if not os.path.isfile(script_path):
            self.output_text.insert(tk.END, f"Errore: script {script_name} not found in dir bin.\n")
            return
        ...
        try:
            ...
            self.output_text.insert(tk.END, f"Error in execution of {script_name}: {e.stderr}\n")
